...
OptiToken utilise des structs Go typées pour parser le token usage depuis les réponses des providers. Quand un provider sort un **nouveau modèle qui change son format** de réponse (ex : `input_tokens` au lieu de `prompt_tokens`, comme Anthropic l'a déjà fait), l'extraction échoue silencieusement :

- Télémétrie faussée
- Économies non calculées
- Métriques dashboard corrompues

Puisqu'on dispose déjà d'un **dropdown de modèles par provider** (via `FetchModelsHandler`), on connaît exactement la liste des modèles existants. On peut donc détecter qu'un modèle **jamais vu** commence à passer par le proxy, et s'y adapter automatiquement.
...
```
Couche 1 — Détection       : Detect unknown model on first request (✅ shipped)
Couche 2 — Collecte        : Accumulate up to 10 response samples in Redis (✅ shipped)
Couche 3 — Auto-diagnostic : Recursive JSON walk to auto-detect usage field paths (✅ shipped)
```
...
Request → ProxyHandler
              ↓
         [Model Radar] ← vérifie si model ∈ known_models[provider]
              ↓ si NOUVEAU modèle
         Flag "unseen_model" dans Redis
              ↓
         → upstream API → streamResponse → ExtractUsage
                                              ↓
                               ...
                               Génère nouveau "usage_mapping" pour ce modèle
                                              ↓
                               Persiste dans Redis (optitoken:radar:usage_mappings)
                                              ↓
...
| Clé | Type | Contenu |
|-----|------|---------|
| `optitoken:radar:known_models` | Set | Modèles connus, seedés depuis `FetchModels` |
| `optitoken:radar:models:<modelID>` | String (JSON) | `RadarEntry` : `firstSeen`, `lastSeen`, `status` (`learning` / `mapped`), `UsageMap`, `SampleCnt` |
| `optitoken:radar:samples:<modelID>` | List | Dernières 10 réponses brutes (LPUSH + LTRIM) |
| `optitoken:radar:usage_mappings` | Hash | `{modelID}` → JSON `{prompt_field, completion_field, confidence_score, sample_count, discovered_at}` |
...
- `CheckAndFlagNewModel(ctx, rdb, provider, modelID) bool` — appelé au début de chaque `ProxyHandler`, retourne `true` si le modèle est inconnu. Crée l'entrée `optitoken:radar:models:<modelID>` avec `status: "learning"`.
- `CollectSample(ctx, rdb, modelID, rawResponse)` — `LPUSH` dans `optitoken:radar:samples:<modelID>` + `LTRIM 0 9` (max 10 samples).
...
**Stratégie** : chercher les champs dont le **nom** contient `prompt`, `input`, `completion`, `output`, `token`, `usage` et dont la **valeur** est un entier > 0. Score = `hitRatio × (1 si valeur > 0, sinon 0.3)`. Seuil : `confidence ≥ 0.5`.
...
- L'auto-discovery tourne et persiste le mapping dans Redis **automatiquement** (confidence ≥ 0.5).
- Le `RadarEntry.status` passe à `"mapped"`.
...
- Le dashboard montre un `costSaved` à 0 pour ces requêtes — c'est honnête (on n'invente pas un chiffre).
- Dès que `FieldDiscoverer` trouve un mapping avec confiance ≥ 0.5, il est persisté et toutes les requêtes suivantes utilisent le nouveau mapping.
...
- Le seed est mis à jour dans le code au fil des releases.
- `FetchModelsHandler` (déjà shippé) ajoute dynamiquement les modèles que le provider expose via son endpoint `/v1/models`.
...
→ À évaluer selon la charge Hetzner actuelle. **Non prioritaire** tant que le FieldDiscoverer JSON couvre les cas courants (en pratique, c'est ~95% des cas).
...
1. ✅ Créer une clé avec un modèle non référencé (ex: `gpt-5-hypothetical`)
2. ✅ Envoyer 5 requêtes
3. ✅ Vérifier Redis : `optitoken:radar:models:*` créé, `optitoken:radar:samples:*` rempli (max 10)
4. ✅ Vérifier dashboard : modèle en statut `learning`
5. ✅ Après 3+ samples → mapping proposé avec confidence score
6. ⚠️ Le panneau dashboard visuel **n'est pas encore implémenté** — utiliser `GET /api/admin/model-radar`
...
- **Confiance trop basse sur certains providers** : si le provider a des fields qui ne matchent aucun de nos keywords (ex: `input_text_tokens` au lieu de `input_tokens`), on retourne `confidence < 0.5` et on abandonne. Le radar entry reste en `learning` jusqu'à intervention manuelle.
...
- [x] Modifier `telemetry.go` — appel à `TryDiscoverForModel` après `CollectSample` (~30min) **shipped**
- [x] `GET /api/admin/model-radar` (~1h) **shipped**
- [ ] Dashboard panneau visuel (~2h)
- [ ] Tests unitaires FieldDiscoverer (~1h) — tests manuels OK, unitaires à ajouter

---

*Dernière mise à jour : 2026-06-16 — Model Radar v1 shipped in prod*
