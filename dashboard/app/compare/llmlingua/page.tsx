...
import { Cpu, Play, Check, Layers } from "lucide-react";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const t = await getTranslation("llmlingua", lang);
  return {
    title: t.metaTitle,
...
export default async function CompareLlmLinguaPage() {
  const cookieStore = cookies();
  const lang = (cookieStore.get("lang")?.value as Language) || "fr";
  const t = await getTranslation("llmlingua", lang);
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans relative overflow-hidden p-8 lg:p-16">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="text-center mb-16">
          <div className="w-16 h-16 mx-auto rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-400 mb-6 shadow-[0_0_30px_rgba(34,211,238,0.2)]">
            <Cpu className="w-8 h-8" />
          </div>
          <h1 className="text-4xl lg:text-6xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-purple-300 to-cyan-500 mb-6">
            {t.heroTitle}
          </h1>
          ...
          {t.sections.map((sec, idx) => (
            <div key={idx} className="bg-[#0f0f11]/60 border border-white/5 rounded-2xl p-8 backdrop-blur-xl">
              ...
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
                {idx === 0 ? <Cpu className="w-5 h-5 text-cyan-400" /> : <Layers className="w-5 h-5 text-purple-400" />} {sec.title}
              </h3>
              ...
              <ul className="space-y-2 text-xs text-gray-300">
                {sec.items.map((item, i) => (
                  <li key={i} className="flex items-center gap-2">
                    <Check className="w-3.5 h-3.5 text-cyan-400" /> {item}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            <div>
              <h3 className="text-2xl font-bold mb-4 flex items-center gap-2">
                <Play className="w-6 h-6 text-cyan-400" /> {t.videoTitle}
              </h3>
              <p className="text-gray-400 text-sm leading-relaxed mb-6">
                {t.videoDesc}
              </p>
            ...
            <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-black/60 aspect-video flex items-center justify-center">
              <DemoVideo src={t.videoUrl || "/token_compression_analytics.webp"} alt={t.videoAlt} />
            </div>
          </div>
        </div>
      </div>
