...
| **Tool-call fingerprinting** (`optiagent/tool_fingerprint.go` + `transport_http.go`) | Proxy-side | Detects an agent calling the same tool with the same arguments twice within 30 seconds. Returns **HTTP 429 + Retry-After: 30** ("Recursive Loop Detected") instead of HTTP 400, so the agent backs off without dying. |
...
- **Agent Firewall as a first-class concept** — every virtual key has 8 firewall fields (`enableL1/2/3`, `killSwitch`, `sessionTokenLimit`, `allowedTools`, `blockUnknownTools`, `redactPII`). Persisted in Postgres, mirrored to Redis on every change so the proxy's hot path never has to hit the DB.
- **Tool-call fingerprinting** complements the loop kill switch. Where the kill switch trips after 3 identical *bodies*, the fingerprint trips after 2 identical *(tool, args)* pairs within 30 s. The kill switch returns `HTTP 400` (the agent is dead); the fingerprint returns `HTTP 429 + Retry-After: 30` (the agent should back off). Most agent frameworks (Claude Code, Cursor, Continue) handle 429 natively as "slow down".
