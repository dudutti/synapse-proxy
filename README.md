...
- **Three-tier cache pipeline (L1/L2/L3):**
  ...
  - **L2 Semantic Match:** Vector search over a 384-dim embedding of the **last user message** (system prompts, tool calls and tool messages are stripped before embedding so the vector always represents the current question). The embedding is produced by an in-process Rust + CGo binding around `paraphrase-multilingual-MiniLM-L12-v2` (the `rust-embedder` directory, compiled to `librust_embedder.a` at build time). The vector is stored in a RediSearch index `idx:l2cache` with cosine distance. Hit threshold: `score < semanticTolerance` (default 0.15, equivalent to cosine similarity > 0.85). Multi-turn chats are NOT disabled from L2 (the older `forceDisableL2` heuristic on tool_calls was removed in production; only the `X-SynapseProxy-Session` header from the dashboard's "Record Session" still disables L2 to prevent cross-turn bleed-through).
  - **L3 Compression:** the byte-preserving compressor described in the L3 section below. CCR semantic-cache hits from tool-call deduplication also land in L3.
...
- The `L3` cache label in the dashboard is set on the byte-preserving compression path only. CCR semantic cache hits still get `L3`, but they go through the embedder-based semantic match described above.
