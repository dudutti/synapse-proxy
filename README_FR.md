...
| **Tool-call fingerprinting** (`optiagent/tool_fingerprint.go` + `transport_http.go`) | Côté proxy | Détecte un agent qui appelle le même outil avec les mêmes arguments 2 fois en 30 s. Renvoie **HTTP 429 + Retry-After: 30** ("Recursive Loop Detected") au lieu d'un HTTP 400, donc l'agent fait un backoff au lieu de mourir. |
...
- **Agent Firewall en concept de première classe** — chaque virtual key a 8 champs firewall (`enableL1/2/3`, `killSwitch`, `sessionTokenLimit`, `allowedTools`, `blockUnknownTools`, `redactPII`). Persistés en Postgres, mirrorés en Redis à chaque changement pour que le proxy n'ait jamais besoin de taper dans la DB.
- **Tool-call fingerprinting** complète le loop kill switch. Là où le kill switch se déclenche après 3 *bodies* identiques, le fingerprint se déclenche après 2 paires *(tool, args)* identiques en 30 s. Le kill switch renvoie `HTTP 400` (l'agent est mort) ; le fingerprint renvoie `HTTP 429 + Retry-After: 30` (l'agent doit ralentir). La plupart des frameworks agent (Claude Code, Cursor, Continue) gèrent le 429 nativement comme "slow down".
